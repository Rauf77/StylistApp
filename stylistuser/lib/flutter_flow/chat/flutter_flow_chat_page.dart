...
import 'package:stylistuser/stylistprofile/stylistprofile_widget.dart';
...
class _FFChatPageState extends State<FFChatPage> {
  ...
  StreamSubscription<List<ChatMessagesRecord>> getMessagesStream(
      ...
      FFChatManager.instance.getChatMessages(chatReference).listen((m) {
        ...
        if (mounted) {
          if(m.length==0){
            sendAutoMessage(text: "Hi..What is your height?");
          }
          if(m.length==2){
            sendAutoMessage(text: "What is your Weight?");
          }
          if(m.length==4){
            sendAutoMessage(text: "What is your age?");
          }
          if(m.length==6){
            sendAutoMessage(text: "What is your Gender?");
          }
          if(m.length==8){
            sendAutoMessage(text: "Thank you for your information. i will get in touch with you soon");
          }
          updateMessages(m);
  ...
  Future sendAutoMessage({String text, String imageUrl}) async {
    if (chatReference == null) {
      return;
    }
    final chatMessagesRecordData = createChatMessagesRecordData(
      user: currentStylist.reference,
      chat: chatReference,
      text: text,
      image: imageUrl,
      timestamp: getCurrentTimestamp,
...
extension _ChatUserExtensions on StyleListRecord {
