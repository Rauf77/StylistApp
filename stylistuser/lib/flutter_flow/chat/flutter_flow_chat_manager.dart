...
class FFChatInfo {
  ...
  final List<StyleListRecord> groupMembers;

  StyleListRecord get currentUser => groupMembers
      .where((user) => user.reference == currentUserReference)
      .first;
  Map<String, StyleListRecord> get otherUsers => Map.fromEntries(
        groupMembers
            .where((user) => user.reference != currentUserReference)
            .map((user) => MapEntry(user.reference.id, user)),
      );
  List<StyleListRecord> get otherUsersList => otherUsers.values.toList();
...
class FFChatManager {
  ...
  Stream<FFChatInfo> getChatInfo({
    StyleListRecord otherUserRecord,
    DocumentReference chatReference,
    ...
    return chatStream.asyncMap((chat) async {
      ...
      final groupMembers = await Future.wait(
        userRefs.map((ref) => StyleListRecord.getDocument(ref).first),
      );
